...
//********************************************************//
//   Funktion tarkoitus: Etsit‰‰n DS18X20 sensoreita      // 
//   1-Wire-v‰yl‰lt‰. diff on tulos viimeisimm‰st‰ rom-   //
//   searchista. *diff = OW_SEARCH_FISRT ensimm‰isell‰    //
//   kutsulla. id on lˆydetyn sensorin rom-koodi.		  //
//********************************************************//
...
//********************************************************//
//   Funktion tarkoitus: Saadaan tiet‰‰, onko anturin     // 
//       virta otettu datalinjalta vai ulkoisesti         //
//********************************************************//

 //sis‰‰ntulo :  id = rom_code 
 //palautetaan:  DS18X20_POWER_EXTERN tai DS18X20_POWER_PARASITE */
...
//********************************************************//
//   Funktion tarkoitus: Aloitetaan mittaus (CONVERT_T)   // 
//   kaikille sensoreille jos si‰‰ntuleva id==NULL tai    //
//   tai yksitt‰iselle sensorille jossa id on rom-koodi   //
//********************************************************//
...
uint8_t DS18X20_start_meas( uint8_t with_power_extern, uint8_t id[])
{
	...
	ow_reset();
	if( ow_input_pin_state() ) { // l‰hetet‰‰n vain jos v‰yl‰ on "joutilaana" = ylh‰‰ll‰
...
//********************************************************//
//   Funktion tarkoitus: Palautetaan 1 jos muunnos on     // 
//   meneill‰‰n, palautetaan 0 jos se on p‰‰ttynyt        //
//   !Ei k‰ytˆss‰, jos virta otetaan datalinjalta         //
//********************************************************//
...
static int32_t DS18X20_raw_to_maxres( uint8_t familycode, uint8_t sp[] )
{
	...
	if ( measure & 0x8000 )  {
		...
		measure ^= 0xffff;  // muunna positiiviseksi => (kahden kÛmplementti)++
		measure++;
	}
	else {
		negative = 0;
...
uint8_t DS18X20_format_from_maxres(long temperaturevalue, char str[])
{
	...
	// alue -550000:-55.0000∞C ... 1250000:+125.0000∞C -> v‰h. 9+1 chars
...
//***************************************************************//
//  Funktion tarkoitus: Komennolla DS18X20_WRITE_SCRATCHPAD      // 
//  voidaan kirjoittaa tietoa anturin RAM-muistiin. Ensimm‰inen  //
//  tieto kirjoitetaan muistipaikkaan kaksi ja seuraavat tiedot  //
//  menev‰t aina seuraaviin muistipaikkoihin                     //
//***************************************************************//
	...
	uint8_t id[] )
{
	...
	if( ow_input_pin_state() ) { // l‰hetet‰‰n vain jos v‰yl‰ on "joutilaana" = ylh‰‰ll‰
		...
		_delay_ms(DS18X20_COPYSP_DELAY); // odota 10 ms 
		if ( with_power_extern != DS18X20_POWER_EXTERN ) {
			ow_parasite_disable();
		}
		ret = DS18X20_OK;
